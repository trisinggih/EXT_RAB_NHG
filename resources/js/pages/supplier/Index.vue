...
const handleSupplierDeletion = (supID: number) => {
    if (confirm('Are you sure you want to delete this role?')) {
        router.delete(route('suppliers.destroy', { id: supID }), {
            preserveScroll: true,
            onSuccess: () => {
                suppliers.value = suppliers.value.filter(supplier => supplier.id !== supID)
            },
            onError: (err) => {
...
const filtered = computed(() => {
  ...
  return source.value.filter((u) => {
    const hay = `${norm(u.id)} ${norm(u.role)} ${norm(u.description)} `
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="mb-6 ">
            <h1 class="text-lg font-semibold text-white bg-primary px-5 py-3 rounded-se-xl shadow border-b-2 border-secondary">
                Supplier List
                <p class="text-white mt-1 text-sm font-normal">Kelola daftar supplier Anda.</p>
            </h1>
        ...
        <div class="flex items-center gap-2">
            ...
            <Link :href="route('suppliers.create')">
              <button class=" btn bg-primary p-2 text-white rounded ml-2">Tambah Supplier</button>
            </Link>
        </div>
        ...
        <table class="w-full text-sm">
            ...
            <tr class="text-gray-700">
                ...
                <th class="px-4 py-3 border-b border-gray-200 text-left">Supplier</th>
                <th class="px-4 py-3 border-b border-gray-200 text-left">Telp</th>
                <th class="px-4 py-3 border-b border-gray-200 text-left">Address</th>
            ...
            >
                ...
                <td class="px-4 py-3 border-b border-gray-200 whitespace-nowrap">{{ u.telp ?? '-' }}</td>
                <td class="px-4 py-3 border-b border-gray-200 whitespace-nowrap">{{ u.address ?? '-' }}</td>
                <td class="px-4 py-3 border-b border-gray-200 whitespace-nowrap">
                    <div class="flex gap-1">
                        <Link :href="route('suppliers.material', {id: u.id})">
                          <Button class="bg-blue-500 text-white">
                              Material
                          </Button>
                        </Link>
                        <Link :href="route('suppliers.edit', {id: u.id})">
                        ...
                        <Button class="bg-red-600 text-white" @click="handleSupplierDeletion(u.id)">
