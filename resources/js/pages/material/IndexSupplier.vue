...
const handlePekerjaanDeletion = (clientID: number) => {
    if (confirm('Are you sure you want to delete this data?')) {
        router.delete(route('supplier.materials.destroy', { id: clientID }), {
            preserveScroll: true,
            onSuccess: () => {
                materials.value = materials.value.filter(pkr => pkr.id !== clientID)
            },
            onError: (err) => {
    ...
    <SupplierLayout :breadcrumbs="breadcrumbs">
        ...
        <div class="flex items-center gap-2">
            ...
            <Link :href="route('supplier.materials.create')">
              <button class=" btn bg-primary p-2 text-white rounded ml-2">Tambah Material</button>
            </Link>
