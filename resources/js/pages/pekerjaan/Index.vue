...
const props = defineProps({
  pekerjaan: { type: Object, default: null }
})
...
onMounted(async () => {
  if (!pekerjaans.value.length) {
    try {
      const res = await axios.get(route('pekerjaan.index'))
      pekerjaans.value = res.data.data
    } catch (e) {
      console.error(e)
    }
  }
...
const source = computed(() => pekerjaans.value)


const handlePekerjaanDeletion = (clientID: number) => {
    if (confirm('Are you sure you want to delete this data?')) {
        router.delete(route('pekerjaans.destroy', { id: clientID }), {
            preserveScroll: true,
            onSuccess: () => {
                pekerjaans.value = pekerjaans.value.filter(pkr => pkr.id !== clientID)
            },
            onError: (err) => {
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="mb-6 ">
            <h1 class="text-lg font-semibold text-white bg-primary px-5 py-3 rounded-se-xl shadow border-b-2 border-secondary">
                Pekerjaan List
                <p class="text-white mt-1 text-sm font-normal">Kelola daftar pekerjaan Anda.</p>
            </h1>
        ...
        <div class="flex items-center gap-2">
            ...
            <Link :href="route('pekerjaan.create')">
              <button class=" btn bg-primary p-2 text-white rounded ml-2">Tambah Pekerjaan</button>
            </Link>
        ...
        <table class="w-full text-sm">
            ...
            >
                ...
                <td class="px-4 py-3 border-b border-gray-200 whitespace-nowrap">
                    <div class="flex gap-1" v-if="u.id !== 2">
                        <Link :href="route('pekerjaan.detail', {id: u.id})">
                          <Button class="bg-cyan-500 text-white">
                              Detail
                          </Button>
                        </Link>
                        <Link :href="route('pekerjaan.edit', {id: u.id})">
