...
const props = defineProps({
  banners: { type: Object, default: null }
})
...
onMounted(async () => {
  if (!banners.value.length) {
    try {
      const res = await axios.get(route('banners.index'))
...
const source = computed(() => banners.value)
...
const handleBannerDeletion = (bannerID: number) => {
    if (confirm('Are you sure you want to delete this data?')) {
        router.delete(route('banners.destroy', { id: bannerID }), {
            preserveScroll: true,
            onSuccess: () => {
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        ...
        <div class="flex items-center gap-2">
            ...
            <Link :href="route('banners.create')">
              <button class=" btn bg-primary p-2 text-white rounded ml-2">Tambah Banner</button>
            </Link>
        ...
        <table class="w-full text-sm">
            ...
            <tr class="text-gray-700">
                ...
                <th class="px-4 py-3 border-b border-gray-200 text-left">Image</th>
                <th class="px-4 py-3 border-b border-gray-200 text-left">Link</th>
            ...
            >
                ...
                <td class="px-4 py-3 border-b border-gray-200 whitespace-nowrap font-medium text-gray-800">{{ u.title_banner }}</td>
                <td class="px-4 py-3 border-b border-gray-200 whitespace-nowrap">
                  <img :src="`/images/${u.img_banner}`" class="w-[100px]" /></td>
                <td class="px-4 py-3 border-b border-gray-200 whitespace-nowrap">{{ u.link_banner ?? '-' }}</td>
                <td class="px-4 py-3 border-b border-gray-200 whitespace-nowrap">
                <div class="flex gap-1">
                        <Button class="bg-red-600 text-white" @click="handleBannerDeletion(u.id)">
